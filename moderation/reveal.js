...
module.exports = {
  ...
  usage: 'reveal',
  help: [
    { name: 'reveal', description: 'Reveal a channel to a member or role.', aliases: 'n/a', parameters: '<channel> <target> <reason>', information: 'n/a', usage: 'reveal <channel> <target> <reason>', example: 'reveal' }
  ],

  run: async (client, message, args) => {

    if (!needPerm(message, 'ManageChannels', 'manage_channels')) return;
    return info(message, `reveal`, `Reveal a channel to a member or role. (params: channel, target, reason)`);
  }
};
