...
module.exports = {
  name: 'unjaildm',
  category: 'moderation',
  usage: 'unjaildm <enable|disable|view>',
  help: [{ name: 'unjaildm', description: 'Toggle whether the bot DMs a member when they are unjail-ed', aliases: 'n/a', parameters: '<enable|disable|view>', information: 'MANAGE_GUILD', usage: 'unjaildm <enable|disable|view>', example: 'unjaildm enable' }],
  run: async (client, message, args) => {
    if (!needPerm(message, 'ManageGuild', 'manage_guild')) return;
    const sub = (args[0] || 'view').toLowerCase();
    const key = `unjaildm_${message.guild.id}`;
    if (sub === 'enable' || sub === 'on') { db.set(key, true); return ok(message, 'DM on \`unjail\` is now **enabled**.'); }
    if (sub === 'disable' || sub === 'off') { db.set(key, false); return ok(message, 'DM on \`unjail\` is now **disabled**.'); }
    if (sub === 'view' || sub === 'status') {
      const v = db.get(key);
      return info(message, 'unjaildm', `DM on \`unjail\`: **${v ? 'enabled' : 'disabled'}**`);
    }
    return warn(message, 'Use `unjaildm enable`, `unjaildm disable`, or `unjaildm view`.');
  }
};
