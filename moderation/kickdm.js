...
module.exports = {
  name: 'kickdm',
  category: 'moderation',
  usage: 'kickdm <enable|disable|view>',
  help: [{ name: 'kickdm', description: 'Toggle whether the bot DMs a member when they are kick-ed', aliases: 'n/a', parameters: '<enable|disable|view>', information: 'MANAGE_GUILD', usage: 'kickdm <enable|disable|view>', example: 'kickdm enable' }],
  run: async (client, message, args) => {
    if (!needPerm(message, 'ManageGuild', 'manage_guild')) return;
    const sub = (args[0] || 'view').toLowerCase();
    const key = `kickdm_${message.guild.id}`;
    if (sub === 'enable' || sub === 'on') { db.set(key, true); return ok(message, 'DM on \`kick\` is now **enabled**.'); }
    if (sub === 'disable' || sub === 'off') { db.set(key, false); return ok(message, 'DM on \`kick\` is now **disabled**.'); }
    if (sub === 'view' || sub === 'status') {
      const v = db.get(key);
      return info(message, 'kickdm', `DM on \`kick\`: **${v ? 'enabled' : 'disabled'}**`);
    }
    return warn(message, 'Use `kickdm enable`, `kickdm disable`, or `kickdm view`.');
  }
};
