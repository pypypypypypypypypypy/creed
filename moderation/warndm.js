...
module.exports = {
  name: 'warndm',
  category: 'moderation',
  usage: 'warndm <enable|disable|view>',
  help: [{ name: 'warndm', description: 'Toggle whether the bot DMs a member when they are warn-ed', aliases: 'n/a', parameters: '<enable|disable|view>', information: 'MANAGE_GUILD', usage: 'warndm <enable|disable|view>', example: 'warndm enable' }],
  run: async (client, message, args) => {
    if (!needPerm(message, 'ManageGuild', 'manage_guild')) return;
    const sub = (args[0] || 'view').toLowerCase();
    const key = `warndm_${message.guild.id}`;
    if (sub === 'enable' || sub === 'on') { db.set(key, true); return ok(message, 'DM on \`warn\` is now **enabled**.'); }
    if (sub === 'disable' || sub === 'off') { db.set(key, false); return ok(message, 'DM on \`warn\` is now **disabled**.'); }
    if (sub === 'view' || sub === 'status') {
      const v = db.get(key);
      return info(message, 'warndm', `DM on \`warn\`: **${v ? 'enabled' : 'disabled'}**`);
    }
    return warn(message, 'Use `warndm enable`, `warndm disable`, or `warndm view`.');
  }
};
