...
module.exports = {
  name: 'timeoutdm',
  category: 'moderation',
  usage: 'timeoutdm <enable|disable|view>',
  help: [{ name: 'timeoutdm', description: 'Toggle whether the bot DMs a member when they are timeout-ed', aliases: 'n/a', parameters: '<enable|disable|view>', information: 'MANAGE_GUILD', usage: 'timeoutdm <enable|disable|view>', example: 'timeoutdm enable' }],
  run: async (client, message, args) => {
    if (!needPerm(message, 'ManageGuild', 'manage_guild')) return;
    const sub = (args[0] || 'view').toLowerCase();
    const key = `timeoutdm_${message.guild.id}`;
    if (sub === 'enable' || sub === 'on') { db.set(key, true); return ok(message, 'DM on \`timeout\` is now **enabled**.'); }
    if (sub === 'disable' || sub === 'off') { db.set(key, false); return ok(message, 'DM on \`timeout\` is now **disabled**.'); }
    if (sub === 'view' || sub === 'status') {
      const v = db.get(key);
      return info(message, 'timeoutdm', `DM on \`timeout\`: **${v ? 'enabled' : 'disabled'}**`);
    }
    return warn(message, 'Use `timeoutdm enable`, `timeoutdm disable`, or `timeoutdm view`.');
  }
};
