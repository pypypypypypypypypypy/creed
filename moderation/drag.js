...
module.exports = {
  ...
  usage: 'drag',
  help: [
    { name: 'drag', description: 'Drag a member to a voice channel. If no voice channel is parsed, then the member is going to be dragged in your voice channel', aliases: 'n/a', parameters: '<member> <voice or channel>', information: 'n/a', usage: 'drag <member> <voice or channel>', example: 'drag' }
  ],

  run: async (client, message, args) => {

    if (!needPerm(message, 'MoveMembers', 'move_members')) return;
    return info(message, `drag`, `Drag a member to a voice channel. If no voice channel is parsed, then the member is going to be dragged in your voice channel (params: member, voice or channel)`);
  }
};
