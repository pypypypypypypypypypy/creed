...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    try {
      ...
      const e = new EmbedBuilder().setColor('#FFFFFF').setTitle(`${a.name} — ${a.artists.map(x=>x.name).join(', ')}`).setURL(a.external_urls.spotify).setDescription(`Released: <t:${Math.floor(new Date(a.release_date).getTime()/1000)}:D>\nTracks: ${a.total_tracks}`).setThumbnail(a.images[0]?.url);
