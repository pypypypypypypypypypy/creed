...
module.exports = {
  ...
  category: 'utility',
  usage: 'grailed <query>',
  help: [{ name: 'grailed', description: 'Search Grailed listings', aliases: 'n/a', parameters: '<query>', information: 'n/a', usage: 'grailed <query>', example: 'grailed rick owens' }],
  run: async (client, message, args) => {
    const q = args.join(' ').trim();
    if (!q) return warn(message, 'Usage: `grailed <query>`');
    const url = `https://www.grailed.com/shop?query=${encodeURIComponent(q)}`;
    return info(message, `Grailed: ${q}`, `[Open results on Grailed](${url})\nGrailed has no public API; results require opening the page.`);
  }
};
