...
module.exports = {
  ...
  category: 'utility',
  usage: 'icon [server]',
  help: [{ name: 'icon', description: 'Show the current server icon', aliases: 'guildicon, servericon', parameters: 'n/a', information: 'n/a', usage: 'icon', example: 'icon' }],
  aliases: ['guildicon','servericon'],
  run: async (client, message, args) => {
    const url = message.guild.iconURL({ size: 1024, extension: 'png' });
    if (!url) return warn(message, 'This server has no icon set.');
    const e = new EmbedBuilder().setColor('#FFFFFF').setTitle(message.guild.name).setImage(url).setURL(url).setTimestamp();
