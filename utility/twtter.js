...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    try {
      ...
      const e = new EmbedBuilder().setColor('#FFFFFF').setAuthor({ name: `${t.user_name} (@${t.user_screen_name})` }).setURL(t.tweetURL).setDescription((t.text || '').slice(0, 4000)).setTimestamp(new Date(t.date || Date.now())).setFooter({ text: `❤ ${t.likes||0} • 🔁 ${t.retweets||0} • 💬 ${t.replies||0}` });
