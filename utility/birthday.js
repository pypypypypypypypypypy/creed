  ...
  run: async (client, message, args) => {
    ...
    if (sub === 'list') {
      ...
      const buildRow = (i, disabled = false) => new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId('bd_prev').setLabel('<').setStyle(ButtonStyle.Secondary).setDisabled(disabled || i === 0),
        new ButtonBuilder().setCustomId('bd_stop').setLabel('✕').setStyle(ButtonStyle.Danger).setDisabled(disabled),
        new ButtonBuilder().setCustomId('bd_next').setLabel('>').setStyle(ButtonStyle.Secondary).setDisabled(disabled || i === chunks.length - 1),
      );
