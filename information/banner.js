...
module.exports = {
  name: 'banner', category: 'information', usage: 'banner [member]',
  help: [{ name: 'banner', description: "Show a member's banner", aliases: 'n/a', parameters: '[member]', information: 'n/a', usage: 'banner [member]', example: 'banner @user' }],
  run: async (client, message, args) => {
    let target = getMember(message, args)?.user || message.author;
    try { target = await client.users.fetch(target.id, { force: true }); } catch {}
    const url = target.bannerURL({ size: 1024, extension: 'png' });
    if (!url) return warn(message, "That user has no banner set.");
    const e = new EmbedBuilder().setColor('#3498db').setTitle(`${target.tag}'s banner`).setURL(url).setImage(url);
