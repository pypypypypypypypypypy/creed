...
module.exports = {
  name: 'bots', category: 'information', usage: 'bots',
  help: [{ name: 'bots', description: 'List bots in this server', aliases: 'n/a', parameters: 'n/a', information: 'n/a', usage: 'bots', example: 'bots' }],
  run: async (client, message, args) => {
    await message.guild.members.fetch().catch(() => {});
    const bots = message.guild.members.cache.filter(m => m.user.bot);
    if (!bots.size) return info(message, 'Bots', 'No bots in this server.');
    const lines = [...bots.values()].map(m => `• ${m.user.tag}`);
    return info(message, `Bots (${bots.size})`, lines.join('\n').slice(0, 4000));
  }
};
