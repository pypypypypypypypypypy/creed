...
module.exports = {
  ...
  help: [
    {
        name: 'handhold',
        description: 'Perform the handhold action on a user',
        aliases: 'n/a',
        parameters: '[user]',
        information: 'n/a',
        usage: 'handhold [user]',
  ...
  run: async (client, message, args) => {
    ...
    const gif = await getRpGif('handhold');
    const embed = new EmbedBuilder()
      .setColor(color)
      .setDescription(`**${message.member.displayName}** held hands with **${target.displayName}** 🤝`)
      .setImage(gif || null);
